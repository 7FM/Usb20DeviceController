...
class UsbTxSim : public VerilatorTB<UsbTxSim, TOP_MODULE> {
  ...
  public:
    ...
    bool stopCondition() { return rxState.receivedLastByte || forceStop; }
...
int main(int argc, char **argv) {
    ...
    for (sim.clk12Offset = 0;
         sim.clk12Offset < 4 && testFailed == 0 && !forceStop;
         ++sim.clk12Offset) {
        ...
        for (int it = 0; !forceStop; ++it) {
            ...
            switch (it) {
                ...
                case 0: {
                    ...
                    // Single byte packet that triggers the CRC bitstuffing at
                    // end edge case!
                    sim.txState.dataToSend.push_back(
                        static_cast<uint8_t>(0xF9));
                    std::cout
                        << "Test single data byte CRC16 bitstuffing edge case:"
                        << std::endl;
                    std::cout << "Expected CRC: "
                              << std::bitset<16>(
                                     constExprCRC<static_cast<uint8_t>(0xF9)>(
                                         CRC_Type::CRC16))
                              << std::endl;
                    break;
                }
                ...
                case 1: {
                    sim.txState.dataToSend.push_back(PID_DATA0);
                    // Two byte packet that triggers the CRC bitstuffing at end
                    // edge case!
                    sim.txState.dataToSend.push_back(
                        static_cast<uint8_t>(0xFF));
                    sim.txState.dataToSend.push_back(
                        static_cast<uint8_t>(0xFA));
                    std::cout
                        << "Test 2 data bytes CRC16 bitstuffing edge case:"
                        << std::endl;
                    std::cout << "Expected CRC: "
                              << std::bitset<16>(
                                     constExprCRC<static_cast<uint8_t>(0xFF),
                                                  static_cast<uint8_t>(0xFA)>(
                                         CRC_Type::CRC16))
                              << std::endl;
                    break;
                ...
                case 2: {
                    ...
                    std::cout << "Test 0 data bytes packet edge case:"
                              << std::endl;
                    std::cout
                        << "Expected CRC: "
                        << std::bitset<16>(constExprCRC<>(CRC_Type::CRC16))
                        << std::endl;
                    break;
                }
                ...
                case 3: {
                    ...
                    std::cout
                        << "Expected CRC: "
                        << std::bitset<16>(
                               constExprCRC<static_cast<uint8_t>(0x11),
                                            static_cast<uint8_t>(0x22),
                                            static_cast<uint8_t>(0x33),
                                            static_cast<uint8_t>(0x44),
                                            static_cast<uint8_t>(0x55),
                                            ...
                                            static_cast<uint8_t>(0xBE),
                                            static_cast<uint8_t>(0xEF),
                                            static_cast<uint8_t>(0xFF)>(CRC16))
                        << std::endl;
                    break;
                }
                ...
                case 4: {
                    ...
                    std::cout << "Test data packet with invalid PID "
                                 "\"checksum\" -> we expect keepPacket = 0"
                              << std::endl;
                ...
                case 9: {
                    ...
                    std::cout << "Test handshake packet with invalid PID "
                                 "\"checksum\" -> we expect keepPacket = 0"
                              << std::endl;
                    expectedKeepPacket = false;
                    break;
                ...
                case 10: {
                    ...
                    const uint8_t *rawPtr =
                        reinterpret_cast<const uint8_t *>(&tokenPacket);
                    ...
                    std::cout << "Test sending a setup token packet"
                              << std::endl;
                ...
                case 11: {
                    ...
                    std::cout << "Test sending a setup token packet with bit "
                                 "stuffing right before the crc5"
                              << std::endl;
                ...
                case 12: {
                    ...
                    std::cout << "Test sending a setup token packet with bit "
                                 "stuffing 1 bit before the crc5"
                              << std::endl;
                ...
                case 13:
                    ...
                    case 40:*/
                    {
                        ...
                        std::cout << "Test sending a setup token packet with "
                                     "random addr & ep"
                                  << std::endl;
            ...
            std::cout << "Expected packet size: "
                      << sim.txState.dataToSend.size() << std::endl;
            ...
            // If crc5 patching is enabled then 5 Msb of the last byte will be
            // patched to include the specified crc5 Else the byte wise
            // comparison between sent & received data will fail!
            if (crc5Patching) {
                ...
                uint8_t crc5 =
                    calculateDataCRC(CRC_Type::CRC5, tmpVec, tmpVec.size());
                std::cout << "Expected CRC: " << std::bitset<5>(crc5)
                          << std::endl;

                uint8_t lastByte =
                    sim.txState.dataToSend[sim.txState.dataToSend.size() - 1];
            ...
            if (sim.txState.dataToSend.size() !=
                sim.rxState.receivedData.size()) {
                IosFlagSaver flagSaver(std::cerr);
                std::cerr
                    << "Send and received byte count differs!\n    Expected: "
                    << sim.txState.dataToSend.size()
                    << " got: " << sim.rxState.receivedData.size() << std::endl;
                std::cerr << "        Send Data: ";
                for (size_t i = 0; i < sim.txState.dataToSend.size(); ++i) {
                    std::cerr << "0x" << std::hex
                              << static_cast<int>(sim.txState.dataToSend[i])
                              << ' ';
                }
                std::cerr << std::endl;
            ...
            size_t compareSize = std::min(sim.txState.dataToSend.size(),
                                          sim.rxState.receivedData.size());
            for (size_t i = 0; i < compareSize; ++i) {
                if (sim.txState.dataToSend[i] != sim.rxState.receivedData[i]) {
                    IosFlagSaver flagSaver(std::cerr);
                    std::cerr << "Send and received byte at idx " << i
                              << " differ!\n    Expected: 0x" << std::hex
                              << static_cast<int>(sim.txState.dataToSend[i])
                              << " got: 0x" << std::hex
                              << static_cast<int>(sim.rxState.receivedData[i])
            ...
            if (expectedKeepPacket != sim.rxState.keepPacket) {
                std::cerr << "Keep packet has an unexpected value! Expected: "
                          << expectedKeepPacket
                          << " got: " << sim.rxState.keepPacket << std::endl;
                ++testFailed;
            }
    ...
    exitInnerLoop:
        ...
        std::cout << " for CLK12 offset of "
                  << static_cast<int>(sim.clk12Offset) << std::endl;
    }
