...
template <std::size_t N> struct USBSignal {
...
#define CALCULATE_CRC_FUN_BODY()                                               \
    ...
    const uint16_t crc_polynom =                                               \
        crcType == CRC16 ? crc16_polynom : crc5_polynom;                       \
                                                                               \
    ...
    crcState &= static_cast<uint16_t>(                                         \
        (static_cast<uint32_t>(1) << (shiftAmount + 1)) - 1);                  \
                                                                               \
    return crcState

template <typename T>
static uint16_t calculateCRC(bool initCRC, uint16_t crcState, CRC_Type crcType,
                             const T data, int bitsInData) {
    CALCULATE_CRC_FUN_BODY();
}

template <typename T>
static constexpr uint16_t
calculateCRC_constExpr(bool initCRC, uint16_t crcState, CRC_Type crcType,
                       const T data, int bitsInData) {
    CALCULATE_CRC_FUN_BODY();
}
...
constexpr bool needsBitStuffing_constExpr(uint8_t &oneCounter,
                                          uint8_t dataBit) {
    NEEDS_BIT_STUFFING_FUN_BODY();
}

template <uint8_t... dataBytes> static constexpr int requiredBitStuffings() {
...
#define CRC_HELPER_FUN_BODY(crcFunName)                                        \
    uint16_t crcState = 0;                                                     \
    crcState = crcFunName<uint8_t>(true, crcState, crcType,                    \
    ...
    for (uint8_t data : bytes) {                                               \
        int bitsInData =                                                       \
            dataIdx < byteCount - 1 ? sizeof(data) * 8 : lastDataBitCount;     \
                                                                               \
        crcState =                                                             \
            crcFunName<uint8_t>(false, crcState, crcType, data, bitsInData);   \
        ++dataIdx;                                                             \
    }                                                                          \
                                                                               \
    crcState = ~crcState;                                                      \
    uint16_t correctlyEncodedCRC = 0;                                          \
                                                                               \
    for (int i = crcType == CRC5 ? 5 : (crcType == CRC16 ? 16 : 0); i > 0;     \
         --i) {                                                                \
...
template <class Bytes>
static constexpr uint16_t constExprCRC_helper(CRC_Type crcType, Bytes bytes,
                                              int byteCount) {
...
template <uint8_t... dataBytes>
constexpr uint16_t constExprCRC(CRC_Type crcType) {
    return constExprCRC_helper<std::initializer_list<uint8_t>>(
        crcType, {dataBytes...}, sizeof...(dataBytes));
}

template <> constexpr uint16_t constExprCRC<>(CRC_Type crcType) {
...
/*
static void applyNrziEncode(uint8_t &nrziEncoderState, uint8_t dataBit, uint8_t
*dp, uint8_t *dn) { APPLY_NRZI_ENCODE_FUN_BODY();
}
*/

static constexpr void applyNrziEncode_constExpr(uint8_t &nrziEncoderState,
                                                uint8_t dataBit, uint8_t *dp,
                                                uint8_t *dn) {
...
constexpr auto nrziEncode(uint8_t initialOneCount = 1,
                          uint8_t encoderStartState = 0) {
    constexpr CRC_Type crcType =
        considerCRC
            ? getCRCTypeFromPID(static_cast<PID_Types>(pid), considerCRC)
            : CRC_INVALID;
    ...
    USBSignal<crcBits + lastDataBitCount + sizeof...(dataBytes) * 8 +
              requiredBitStuffings<pid, dataBytes...,
                                   static_cast<uint8_t>(crc & 0x0FF),
                                   static_cast<uint8_t>((crc >> 8) & 0x0FF)>()>
        signal;
    int signalIdx = 0;
    ...
    for (uint8_t data : {pid, dataBytes...}) {
        int bitsInData = dataIdx != sizeof...(dataBytes) - 1 ? sizeof(data) * 8
                                                             : lastDataBitCount;
        ...
        for (int i = bitsInData; i > 0; --i, ++signalIdx) {
            ...
            applyNrziEncode_constExpr(nrziEncoderState, dataBit,
                                      signal.dp + signalIdx,
                                      signal.dn + signalIdx);

            if (needsBitStuffing_constExpr(bitStuffingOneCounter, dataBit)) {
                // If the allowed amount of consecutive one's are exceeded a 0
                // needs to be stuffed into the signal!
                ++signalIdx;
                applyNrziEncode_constExpr(nrziEncoderState, 0,
                                          signal.dp + signalIdx,
                                          signal.dn + signalIdx);
            }
        }
    ...
    for (int crcBitsCopy = crcBits; crcBitsCopy > 0;
         --crcBitsCopy, ++signalIdx) {
        uint8_t dataBit = crcCopy & 1;
        // Next data bit
        crcCopy >>= 1;

        applyNrziEncode_constExpr(nrziEncoderState, dataBit,
                                  signal.dp + signalIdx, signal.dn + signalIdx);

        if (needsBitStuffing_constExpr(bitStuffingOneCounter, dataBit)) {
            // If the allowed amount of consecutive one's are exceeded a 0 needs
            // to be stuffed into the signal!
            ++signalIdx;
            applyNrziEncode_constExpr(nrziEncoderState, 0,
                                      signal.dp + signalIdx,
                                      signal.dn + signalIdx);
...
template <class... SignalPart> static constexpr int determineSignalLength() {
    return (0 + ... + SignalPart::size);
}

template <class SignalPart, std::size_t N>
static constexpr void constructSignalHelper(const SignalPart &signalPart,
                                            int &idx,
...
constexpr auto usbSyncSignal =
    nrziEncode<false, static_cast<uint8_t>(0b1000'0000)>(0, 1);
...
void receiveDeserializedInput(const Sim &sim, T *top,
                              UsbReceiveState &usbRxState, bool posedge,
                              bool negedge) {
    if (posedge) {
        if (top->rxDone) {
            if (usbRxState.receivedLastByte) {
                std::cerr << "Error: got rxDone signal multiple times!"
                          << std::endl;
            } else {
                usbRxState.keepPacket = top->keepPacket;
                std::cout << "Received last byte! Overall packet size: "
                          << usbRxState.receivedData.size() << std::endl;
                std::cout << "Usb RX module keepPacket: "
                          << usbRxState.keepPacket << std::endl;
            }
        ...
        if (top->rxAcceptNewData && top->rxDataValid) {
            ...
            if (usbRxState.receivedLastByte) {
                std::cerr << "Error: received bytes after last signal was set!"
                          << std::endl;
            }
        }
    } else if (negedge) {
...
struct UsbTransmitState {
  ...
  public:
    ...
    void actAsNop() {
        ...
        // fake that the request send packet was already set -> no new send is
        // initiated
...
void feedTransmitSerializer(T *top, UsbTransmitState &usbTxState) {
    if (usbTxState.requestedSendPacket) {
        top->txIsLastByte =
            usbTxState.transmitIdx == usbTxState.dataToSend.size() - 1 ? 1 : 0;
        ...
        if (top->txAcceptNewData) {
            if (top->txDataValid) {
                ...
                // Only signal data is valid if there is still data left to
                // send!
                if (usbTxState.transmitIdx < usbTxState.dataToSend.size()) {
                    // Data was requested but not yet signaled that txData is
                    // valid, lets change the later
                    top->txDataValid = 1;
                }
            }
        }
